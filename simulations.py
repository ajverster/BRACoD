...
def simulate_microbiome_counts(df_counts, n_contributors = 5, coeff_contributor = 2.0, min_ab_contributor = -9, sd_Y = 1.0, n_reads = 100000, var_contributor = None, use_uniform = False, n_samples_use = None, corr_value = None, var_factor = None, return_absolute = False, seed = None):
    if seed is None:
        np.random.seed(seed)

    df_relab = df_counts.apply(lambda x: x / np.sum(x), 0)
    if n_samples_use is None:
        n_samples_use = df_counts.shape[1]
    # lowest value
